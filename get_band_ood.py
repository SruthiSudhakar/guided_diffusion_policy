...
def get_detection_with_plot(log_probs, successes, log_probs_test, successes_test, img_frames, save_folder, alpha=0.01, lb=False, CPband=True, suffix=''):
    ...
    """
    num_te = len(successes_test) # these are heldout for testing and evaluating failure detection
    max_tr = len(successes) # these are used in CP construction
    ...
    global_indices_of_test = np.arange(len(log_probs_test))
...
def main():
    ...
    successes_test = []
    all_log_probs_test = []
    experiments_test = pickle.load(open(dataset_test_path, 'rb'))
    
    for idx, demo_key in tqdm(enumerate(list(experiments_test['logdata'].keys()))):
        if 'train/sim_max_reward' in demo_key.lower():
            logpzo_scores = experiments_test['logdata'][demo_key][1]
            scores = [float(x) for x in logpzo_scores.split('/')]
            success = experiments_test['logdata'][demo_key][0]
            ...
            all_log_probs_test.append(scores)
            successes_test.append(success)
            # all_images.append(img_frames)
    max_length_test = max(len(lp) for lp in all_log_probs_test)
    padded_log_probs_test = [lp + [lp[-1]] * (max_length_test - len(lp)) for lp in all_log_probs_test]
    log_probs_test = np.array(padded_log_probs_test)
    successes_test = np.array(successes_test)
    print("TEST success rate = ", np.mean(successes_test))
    print("TEST log_probs shape:", log_probs_test.shape)
