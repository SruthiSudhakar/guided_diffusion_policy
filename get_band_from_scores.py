...
def main():
    ...
    for idx, demo_key in tqdm(enumerate(list(experiments['logdata'].keys()))):
        if 'train/sim_max_reward' in demo_key.lower() or 'test/sim_max_reward' in demo_key.lower():
            ...
            # gripper_img_obs = experiments['eyeinhand'][idx]
            # left_img_obs = experiments['leftside'][idx]
            # right_img_obs = experiments['rightside'][idx]
            ...
            #     grippercam = gripper_img_obs[t]
            #     leftcam = left_img_obs[t]
            #     rightcam = right_img_obs[t]

            #     leftcam = leftcam[0,:].detach().cpu().numpy()
            #     rightcam = rightcam[0,:].detach().cpu().numpy()
            #     grippercam = grippercam[0,:].detach().cpu().numpy()
