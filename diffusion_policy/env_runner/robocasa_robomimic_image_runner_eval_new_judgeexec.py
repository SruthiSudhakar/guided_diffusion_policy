...
def get_qwen_relative_rank_batchify(all_video_paths, model, processor, n_envs, PROMPTS):
    ...
    for env_idx, one_env_videos in enumerate(all_video_paths):
        sample_image_paths = [save_last_frame_method(p) for p in one_env_videos]
    ...
    relative_ranks = []
    image_paths_to_annotate = []
    texts_to_add = []
    for k, output in enumerate(all_outputs):
        try:
            value = PROMPTS['extract_function'](output)
        except (ValueError, TypeError) as e:
            print(f"Warning: Failed to parse output '{output}' - Error: {e}")
            value = None
        relative_ranks.append(value)
        image_paths_to_annotate.append(pair_metadata[k]['overlay_image_path'])
        texts_to_add.append(value)

    # Debug: Print distribution of scores
    valid_scores = [s for s in relative_ranks if s is not None and s != 0]
    zero_scores = sum(1 for s in relative_ranks if s == 0)
    none_scores = sum(1 for s in relative_ranks if s is None)
    print(f"Score distribution: {len(valid_scores)} valid, {zero_scores} zeros, {none_scores} None")
    if valid_scores:
        print(f"Valid scores range: [{min(valid_scores):.2f}, {max(valid_scores):.2f}]")

    # Parallelize image annotation (much faster)
                    ...
                    if self.choose_sample and self.start_sampling<env_step_index<self.end_sampling:
                        ...
                        for sample_idx in pbar2:
                            ...
                            for extra_step in range(self.additional_steps):
                                ...
                                # Store the additional sampled actions
                                step2_sampled_actions[extra_step].append(step2_actions)
                        ...
                        # Extract the best initial actions (first 16 actions)
                        selected_initial_actions = actions[np.arange(actions.shape[0]), best_action_indices, :self.num_actions_to_execute, :]

                        # Extract the best additional actions for each extra step
                        all_additional_actions = []
                        for extra_step in range(self.additional_steps):
                            # step2_sampled_actions[extra_step] is a list of length num_samples
                            # Each element has shape (batch_size, 16, 7)
                            step2_actions_array = np.array(step2_sampled_actions[extra_step])  # Shape: (num_samples, batch_size, 16, 7)
                            # Transpose to (batch_size, num_samples, 16, 7) to match the selection logic
                            step2_actions_array = np.transpose(step2_actions_array, (1, 0, 2, 3))
                            # Select the best actions based on best_action_indices
                            selected_step2_actions = step2_actions_array[np.arange(step2_actions_array.shape[0]), best_action_indices, :self.num_actions_to_execute, :]
                            all_additional_actions.append(selected_step2_actions)

                        # Concatenate all actions: initial + all additional steps
                        # selected_initial_actions shape: (batch_size, num_actions_to_execute, 7)
                        # each selected_step2_actions shape: (batch_size, num_actions_to_execute, 7)
                        # Concatenate along the action sequence dimension (dim 1)
                        all_actions_to_execute = np.concatenate([selected_initial_actions] + all_additional_actions, axis=1)
                        # all_actions_to_execute shape: (batch_size, num_actions_to_execute * (1 + additional_steps), 7)

                        # Store all actions for execution
                        action_dict={'action':torch.tensor(all_actions_to_execute).to(device)}
                    else:
