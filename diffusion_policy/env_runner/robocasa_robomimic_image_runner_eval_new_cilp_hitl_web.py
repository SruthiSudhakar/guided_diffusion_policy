...
from hitl_web_selector import get_user_input_web_selection
...
def get_user_input_direct_selection(all_video_paths, exited_envs=None):
    best_indices = []
    raw_results = []
    ...
    for env_idx, one_env_videos in enumerate(all_video_paths):
        if exited_envs is not None and env_idx in exited_envs:
            print(f"Environment {env_idx} already exited. Skipping...")
            best_indices.append('exit')
            scores = [0.0 for _ in range(n_samples)]
        ...
        while True:
            try:
                ...
                print(f"Selected sample index: {user_input}")
                if user_input == 'exit':
                    selected_idx = 'exit'
                    if exited_envs is not None:
                        exited_envs.add(env_idx)
                    break
                    ...
                    if self.choose_sample and self.start_sampling<env_step_index<self.end_sampling:
                        ...
                        for sample_idx, one_of_n_samples in enumerate(sd_sample_obs):
                            for env_idx, one_env in enumerate(one_of_n_samples):
                                ...
                                os.makedirs(f'{self.output_dir}/videos/env_{env_idx}',exist_ok=True)
                                images_to_video_side_by_side(list1_of_images, list2_of_images, list3_of_images, output_path=f'{self.output_dir}/videos/env_{env_idx}/step_{env_step_index}_sample_{sample_idx}.mp4')
                                videos[env_idx].append(f'{self.output_dir}/videos/env_{env_idx}/step_{env_step_index}_sample_{sample_idx}.mp4')                        
                        # flattened_videos_list = list(itertools.chain.from_iterable(videos))
                        # flattened_videos_list = list(itertools.chain.from_iterable(videos))
                        best_action_indices, raw_results = get_user_input_web_selection(videos, exited_envs=exited_envs, step_number=env_step_index)

                        if all(x == 'exit' for x in best_action_indices):
                            print("All users want to exit")
                            done=True
                            break
                        print('best actions idx:', best_action_indices)
                        best_action_indices = [int(x) if x != 'exit' else 0 for x in best_action_indices]
