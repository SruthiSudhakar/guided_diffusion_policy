...
def add_text_to_video_with_ffmpeg(input_video_path, text_arm, text_plate):
    ...
    command = [
        ...
        '-vf', f"drawtext=text='{text_arm}':x=50:y=50:fontsize=24:fontcolor=#800080:borderw=1:bordercolor=#800080,"  # Purple text and border for arm
              f"drawtext=text='{text_plate}':x=50:y=100:fontsize=24:fontcolor=#008000:borderw=1:bordercolor=#008000",  # Green text and border for plate
...
def preprocess_video(video_info, processor,USER_PROMPT_CRITIC):
...
def process_batch(llm, video_batch, processor, sampling_params, USER_PROMPT_CRITIC, plate_or_arm, max_workers=100, chunk_size=100):
    ...
    for i in range(0, len(video_batch), chunk_size):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            preprocessed = list(executor.map(
                lambda v: preprocess_video(v, processor, USER_PROMPT_CRITIC),
                chunk
            ))
        ...
        for j, output in enumerate(outputs):
            ...
            for response in output.outputs:
                ...
                if plate_or_arm=='arm':
                    ...
                    match = re.search(r'Direction:\s*\[(LEFT|RIGHT)\]', generated_text, re.IGNORECASE)
                    if not match:
                        match = re.search(r'Direction:\s*(LEFT|RIGHT)', generated_text, re.IGNORECASE)
                    if match:
                        answer = match.group(1).lower()
                        binary_answers.append(answer)
                elif plate_or_arm=='plate':
                    # Extract binary answer from each response
                    match = re.search(r'Position:\s*\[(LEFT|RIGHT)\]', generated_text, re.IGNORECASE)
                    if not match:
                        match = re.search(r'Position:\s*(LEFT|RIGHT)', generated_text, re.IGNORECASE)
                    if match:
                        answer = match.group(1).lower()
            ...
            # Determine final answer by taking max (LEFT > RIGHT)
            final_answer = None
            if binary_answers:
                left_count = binary_answers.count('left')
                right_count = binary_answers.count('right')
                final_answer = 'left' if left_count >= right_count else ('right' if right_count > left_count else 'Neither')
                    
            all_results[video_path] = {
                'gentext': all_responses,
                'individual_answers': binary_answers,
                'left_count': binary_answers.count('left') if binary_answers else 0,
                'right_count': binary_answers.count('right') if binary_answers else 0,
            ...
            while not done:
                ...
                with torch.no_grad():
                    ...
                    if self.choose_sample and self.start_sampling<env_step_index<self.end_sampling:
                        ...
                        batch_results_arms = process_batch(self.llm, flattened_videos_list, self.processor, self.sampling_params, USER_PROMPT_CRITIC, 'arm')
                        ...
                        batch_results_plates = process_batch(self.llm, flattened_videos_list, self.processor, self.sampling_params, USER_PROMPT_CRITIC, 'plate')
                        best_action_indices=[0]*len(videos)
                        for i, one_video in enumerate(videos):
                            for j, one_sample in enumerate(one_video):
                                add_text_to_video_with_ffmpeg(one_sample,batch_results_arms[one_sample]['binary'],batch_results_plates[one_sample]['binary'])
                                if batch_results_arms[one_sample]['binary']==batch_results_plates[one_sample]['binary']:
                                    best_action_indices[i] = j
                        print('best actions idx:', best_action_indices)
                        chunk_step_actions[chunk_idx][env_step_index]={'best_action_indices':best_action_indices, 'raw_results_arm': batch_results_arms, 'raw_results_plate': batch_results_plates}
