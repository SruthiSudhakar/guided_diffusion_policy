...
class TimmObsEncoder(ModuleAttrMixin):
    def __init__(self,
        ...
        if transforms is not None and not isinstance(transforms[0], torch.nn.Module):
            ...
            transforms = [
                torchvision.transforms.Resize(size=int(image_shape[0] / ratio), antialias=True),
                torchvision.transforms.RandomCrop(size=image_shape),
            ] + transforms[1:]
...
class FrozenOpenCLIPImageEmbedder(nn.Module):
    ...
    def forward(self, tokenized_text): 
        xt, x = self.encode_text(tokenized_text)
        xt = xt.float()
        return xt
    ...
    def encode_text(self, text):
        ...
        x = self.model.token_embedding(tokens).to(cast_dtype)  # [batch_size, n_ctx, d_model]
        ...
        xt = x[torch.arange(x.shape[0]), tokens.argmax(dim=-1)] @ self.model.text_projection
