...
class InMemoryVideoDataset(Dataset):
    ...
    def get_dataset_file(self, task_list, human_paths):
        datasets = []
        hdf5_datasets = []
        for task in task_list:
            if task not in human_paths:
                # If task doesn't have a specific path, skip it
                print(f"Warning: No path specified for task {task}, skipping...")
                continue
            
            task_path = human_paths[task]
            
            # Check if file exists
            if not os.path.exists(task_path):
                raise FileNotFoundError(f"HDF5 file not found for task {task}: {task_path}")
            
            print(f"Loading {task} from {task_path}")
            
            # Load the HDF5 file for this specific task
            in_memory_data = self.load_hdf5_into_memory(task_path)
            datasets.append(in_memory_data)

            hdf5_file = h5py.File(task_path, 'r')
