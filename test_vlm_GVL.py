...
CUDA_VISIBLE_DEVICES=7 python3 test_vlm_GVL.py \
    --image_folder test_images_StoveToCounter \
    --task_token "[STOVE_TO_COUNTER]" \
    --output_dir GVL_20000 \
    --quantization
"""
...
INITIAL_SCENE = """Note that these frames are in random order, so please pay attention to the individual frames when reasoning about task completion percentage."""
...
def run_inference(model, processor, overlay_image, task_token):
    ...
    frame1 = overlay_image.crop((0, 0, overlay_image.width // 2-10, overlay_image.height))
    frame2 = overlay_image.crop((overlay_image.width // 2+10, 0, overlay_image.width, overlay_image.height))
    messages = [
        {"role": "system", "content": [{"type": "text", "text": ""}]},
        {
            "role": "user",
            "content": [
                {"type": "text", "text": IN_CONTEXT_EXAMPLE.replace("{task_token}", TASK_TOKEN_TO_DESC[task_token])},
                {"type": "text", "text": "Initial robot scene"},
                {"type": "image", "image": TASK_TOKEN_TO_CONTEXT[task_token]["initial_scene"]},
                {"type": "text", "text": "Frame 1:"},
                {"type": "image", "image": TASK_TOKEN_TO_CONTEXT[task_token]["frame1"]},
                {"type": "text", "text": "Frame 2:"},
                {"type": "image", "image": TASK_TOKEN_TO_CONTEXT[task_token]["frame2"]},
                {"type": "text", "text": TASK_TOKEN_TO_CONTEXT[task_token]["answer"]},
                {"type": "text", "text": INITIAL_SCENE},
                # {"type": "text", "text": "Initial robot scene"},
                # {"type": "image", "image": initial_scene},
                {"type": "text", "text": USER_PROMPT_TEMPLATE.replace("{task_token}", TASK_TOKEN_TO_DESC[task_token])},
                {"type": "text", "text": "Frame 1:"},
...
def main():
    ...
    parser.add_argument("--model_path", default="/workspace/cosmos-reason1/data/huggingface/transformers/Qwen2.5-VL-7B-Instruct", type=str, help="Path to trained model")
